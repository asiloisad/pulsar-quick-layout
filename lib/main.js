...
module.exports = {
  ...
  addDockButtons() {
    ...
    DOCK_BUTTONS.forEach(({ icon, iconOpen, command, title }, i) => {
      ...
      const updateIcon = (visible) => { btn.innerHTML = visible ? iconOpen : icon; };
      updateIcon(dock.isVisible());
      this.subscriptions.add(dock.onDidChangeVisible(updateIcon));
      this.subscriptions.add(atom.tooltips.add(btn, {
        title,
  ...
  addLayoutButtons() {
    ...
    LAYOUT_BUTTONS.forEach(({ icon, command, title }) => {
      const btn = createButton(icon, command);
      this.subscriptions.add(atom.tooltips.add(btn, {
        title,
        keyBindingCommand: command,
...
const DOCK_BUTTONS = [
  { icon: ICONS.left, iconOpen: ICONS.leftOpen, command: "quick-layout:toggle-left-dock", title: () => `Left dock is ${atom.workspace.getLeftDock().isVisible() ? 'open' : 'closed'}` },
  { icon: ICONS.bottom, iconOpen: ICONS.bottomOpen, command: "quick-layout:toggle-bottom-dock", title: () => `Bottom dock is ${atom.workspace.getBottomDock().isVisible() ? 'open' : 'closed'}` },
  { icon: ICONS.right, iconOpen: ICONS.rightOpen, command: "quick-layout:toggle-right-dock", title: () => `Right dock is ${atom.workspace.getRightDock().isVisible() ? 'open' : 'closed'}` },
];
