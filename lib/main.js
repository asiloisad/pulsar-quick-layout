  ...
  ensurePanes(n, true)
    .then(() => {
      ...
      for (const pane of Array.from(panes)) {
        for (let i = 1; i < n; i++) {
          pane.splitDown({ copyActiveItem: false });
        }
      }
      if (detail?.redistribute) redistributeItems();
      else if (detail?.sequentize) sequentizeItems();
...
const ICONS = {
  ...
  grid2: '<svg width="14" height="14" viewBox="0 0 14 14"><rect x="0.5" y="0.5" width="13" height="13" rx="1" fill="none" stroke="currentColor" stroke-width="1"/><path d="M7 0.5v13M0.5 7h13" stroke="currentColor" stroke-width="1"/></svg>',
  grid3: '<svg width="14" height="14" viewBox="0 0 14 14"><rect x="0.5" y="0.5" width="13" height="13" rx="1" fill="none" stroke="currentColor" stroke-width="1"/><path d="M5 0.5v13M9 0.5v13M0.5 5h13M0.5 9h13" stroke="currentColor" stroke-width="1"/></svg>',
};
...
function createButton(icon, command) {
  ...
  btn.addEventListener("contextmenu", (e) => {
    e.preventDefault();
    atom.commands.dispatch(atom.views.getView(atom.workspace), command, { redistribute: true });
  });
  btn.addEventListener("auxclick", (e) => {
    if (e.button !== 1) return;
    e.preventDefault();
    atom.commands.dispatch(atom.views.getView(atom.workspace), command, { sequentize: true });
  });
  return btn;
}
...
const LAYOUT_BUTTONS = [
  ...
  { icon: ICONS.grid2, command: "quick-layout:grid-two" },
];
