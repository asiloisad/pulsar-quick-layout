  ...
  activate() {
    this.subscriptions = new CompositeDisposable(
      atom.commands.add("atom-workspace", {
        "quick-layout:one-pane": (e) =>
          ensurePanes(1, true, e.detail),
        "quick-layout:two-columns": (e) =>
          ensurePanes(2, true, e.detail),
        "quick-layout:three-columns": (e) =>
          ensurePanes(3, true, e.detail),
        "quick-layout:four-columns": (e) =>
          ensurePanes(4, true, e.detail),
        "quick-layout:two-rows": (e) =>
          ensurePanes(2, false, e.detail),
        "quick-layout:three-rows": (e) =>
          ensurePanes(3, false, e.detail),
        "quick-layout:grid-2x2": (e) =>
          ensureGrid(2, e.detail),
        "quick-layout:grid-3x3": (e) =>
          ensureGrid(3, e.detail),
        "quick-layout:redistribute": () =>
          redistributeItems(),
        "quick-layout:sequentize": () =>
          sequentizeItems(),
        "quick-layout:toggle-left-dock": () =>
          atom.workspace.getLeftDock().toggle(),
        "quick-layout:toggle-right-dock": () =>
          atom.workspace.getRightDock().toggle(),
...
function mergeTwoPanes(lastPane, secondToLastPane) {
  ...
  for (const item of Array.from(lastPaneItems)) {
    lastPane.moveItemToPane(
      item,
      secondToLastPane,
      secondToLastPane.getItems().length
    );
  }
  lastPane.destroy();
}
...
function redistributeItems() {
  ...
  const activeItem = atom.workspace
    .getCenter()
    .getActivePane()
    ?.getActiveItem();
  const allItems = [];
...
async function ensurePanes(n, horizontal, detail) {
  ...
  if (
    allCenterPanes.some(
      (p) => p.parent?.parent?.constructor?.name === "PaneAxis"
    )
  ) {
